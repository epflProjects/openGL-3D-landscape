...
float perlin_noise(float x, float y, float z) {
  ...
  return lerp(
      w,
      lerp(v,
           lerp(u, grad(int(permutation[AA]), x, y, z),        // AND ADD
      ...
      lerp(v,
           lerp(u, grad(int(permutation[AA + 1]), x, y, z - 1),   // CORNERS
...
float RMF(vec3 point, float octaves) {
  float result, signal, weight, frequency = 0.6, H = 1.0, offset = 1.0,
                                gain = 2.0;
  int i;
  for (i = 0; i < octaves; i++) {
...
// Need to have exponent_array initialized according to frequency and
// lacunarity. (done on CPU now)
...
#if PERLIN_NOISE
    value +=
        remainder * perlin_noise(point.x, point.y, point.z) * exponent_array[i];
#else
