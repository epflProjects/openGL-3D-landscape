...
public:
  static GLfoat* g_snowflake_position_size_data = new GLfloat[MaxSnowFlakes * 4];
  ...
  Init() {
      ...
      glBufferData(GL_ARRAY_BUFFER, MaxSnowFlakes * 4 * sizeof(GLfloat), NULL, GL_STREAM_DRAW);
      ...
      glBufferData(GL_ARRAY_BUFFER, MaxSnowFlakes * 4 * sizeof(GLubyte), NULL, GL_STREAM_DRAW);
  }
  ...
  void Draw() {
    ...
    for (int i = 0; i < newsnowflakes; i++) {
      ...
      SnowFlakesContainer[snowflakeIndex].life = 5.0f; // this snowflake will live 5 seconds
      SnowFlakesContainer[snowflakeIndex].pos = glm::vec3(0,0,-20.0f);

      float spread = 1.5f;
      glm::vec3 maindir = glm::vec3(0.0f, 10.0f, 0.0f);

      // bad way to create random direction. See: http://stackoverflow.com/questions/5408276/python-uniform-spherical-distribution
      ...
      SnowFlakesContainer[snowflakeIndex].size = (rand()%1000)/2000.0f + 0.1f;
    }
