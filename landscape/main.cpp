...
vec3 pos[numberOfPts] = {
    vec3(-0.96145, 0.334909, 0.98078),    vec3(-0.660958, 0.110372, 0.658177),
    vec3(-0.572374, 0.0633491, 0.573775), vec3(0.109945, 0.158341, -0.162833),
    vec3(-0.0461158, 0.49035, -2.03308), // out
    vec3(0.570173, 0.0930193, -0.492701), vec3(0.696272, 0.340389, -0.151085),
    vec3(0.873018, 0.423555, 0.982745)};

vec3 look[numberOfPts] = {
    vec3(0.41161, -0.600675, -0.401152),  vec3(0.755826, -0.255177, -0.642288),
    vec3(0.816481, 0.0232589, -0.749613), vec3(1.55104, 0.221676, -1.40737),
    vec3(1.90209, 0.0565342, -2.10169), // out
    vec3(1.0685, 0.16972, 1.39271),       vec3(-0.629362, 0.365474, 1.14593),
    vec3(-0.157628, -0.810295, -0.529818)};

// to store globally the height of the terrain.
GLfloat heightmap_data[tex_width * tex_width];

// debug function, to fflush the error queue to be sure the next error recieved
// afetr this is the one from the function trialed.
...
void framebufferHMRender() {
  ...
  framebuffer.Bind();
  {
    ...
    // stocking the result of the heightmap to a global array, in order to read
    // height values
...
// return the height of the terrain according to the x and y coordinates given
// as parameters.
...
void Init(GLFWwindow *window) {
  ...
  // cout << "test of Read pixels : " << heightmap_data[(tex_width - 1) *
  // (tex_width - 1)] << " and "<< heightmap_data[1] << endl;  enable
  // transparency
  glEnable(GL_BLEND);
...
void move(bool forward, float move_coeff) {
  ...
  total = abs(cam_look[0] - cam_pos[0]) + abs(cam_look[1] - cam_pos[1]) +
          abs(cam_look[2] - cam_pos[2]);
  cam_pos[0] =
      cam_pos[0] + coeff * (((cam_look[0] - cam_pos[0]) / total) * move_coeff);
  cam_pos[2] =
      cam_pos[2] + coeff * (((cam_look[2] - cam_pos[2]) / total) * move_coeff);
  if (!FPS_mode) {
    cam_pos[1] = cam_pos[1] +
                 coeff * (((cam_look[1] - cam_pos[1]) / total) * move_coeff);
  } else {
    dontExceedTerrainIfFPS();
    cam_pos[1] =
        getHeight(cam_pos[0], cam_pos[2], tex_width) / FPS_coeff + margin;
  }
  cam_look[0] =
      cam_look[0] + coeff * (((cam_look[0] - cam_pos[0]) / total) * move_coeff);
  if (!FPS_mode) {
    cam_look[1] = cam_look[1] +
                  coeff * (((cam_look[1] - cam_pos[1]) / total) * move_coeff);
  }
  cam_look[2] =
      cam_look[2] + coeff * (((cam_look[2] - cam_pos[2]) / total) * move_coeff);
}

void moveForward(float move_coeff) { move(true, move_coeff); }

void moveBackward(float move_coeff) { move(false, move_coeff); }

void rotate2D(float position_0, float position_1, float &look0, float &look1,
              float angle, bool vertical) {
  if (!vertical) {
    look0 = position_0 + (look0 - position_0) * cos(angle) -
            (look1 - position_1) * sin(angle);
  }
  look1 = position_1 + (look0 - position_0) * sin(angle) +
          (look1 - position_1) * cos(angle);
}
...
void displayMove() {
  ...
  if (m_right && !inertia) {
    rotate2D(cam_pos[0], cam_pos[2], cam_look[0], cam_look[2],
             default_rotation_move_coeff, false);
  }
  if (m_right && inertia) {
    rotate2D(cam_pos[0], cam_pos[2], cam_look[0], cam_look[2],
             inertia_move_coeff, false);
    inertia_move_coeff /= 1.5;
    if (inertia_move_coeff < 0.00001f) {
      m_right = false;
      inertia = false;
  ...
  if (m_left && !inertia) {
    rotate2D(cam_pos[0], cam_pos[2], cam_look[0], cam_look[2],
             (-1 * default_rotation_move_coeff), false);
  }
  if (m_left && inertia) {
    rotate2D(cam_pos[0], cam_pos[2], cam_look[0], cam_look[2],
             (-1 * inertia_move_coeff), false);
    inertia_move_coeff /= 1.5;
    if (inertia_move_coeff < 0.00001f) {
      m_left = false;
      inertia = false;
  ...
  if (m_up && !inertia) {
    rotate2D(0, 0, cam_look[0], cam_look[1], (-1 * default_rotation_move_coeff),
             true);
  }
  if (m_up && inertia) {
...
vec3 getBezierPositionForFrame(vec3 *pts, int size, float bezier_frame_number) {
  if (size == 1) {
    ...
    for (int i = 0; i < size - 1; ++i) {
      newPts[i] =
          (1 - bezier_frame_number) * pts[i] + bezier_frame_number * pts[i + 1];
    }
    delete[] pts;
...
void Display() {
  ...
  terrain.Draw(time, trackball_matrix * IDENTITY_MATRIX, view_matrix,
               projection_matrix);
  ...
  if (bezier_mode) {
    ...
    if (bezier_frame_number > 1.0f) {
      ...
      cam_pos = getBezierPositionForFrame(pos_points(), numberOfPts,
                                          bezier_frame_number);
      cam_look = getBezierPositionForFrame(look_points(), numberOfPts,
                                           bezier_frame_number);
    }
  } else {
    displayMove();
...
int main(int argc, char *argv[]) {
  ...
  GLFWwindow *window =
      glfwCreateWindow(window_width, window_height, "framebuffer", NULL, NULL);
  if (!window) {
    glfwTerminate();
    return EXIT_FAILURE;
  }
