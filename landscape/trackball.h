...
public:
    ...
    mat4 Drag(float x, float y) {
      ...
      // article.
      // anchor_pos_ and current_pos
      float scalar_factor = 0.75;
      vec3 rotation_axis = cross(anchor_pos_, current_pos);
      float angle = acos(dot(anchor_pos_, current_pos) / (length(anchor_pos_) * length(current_pos)));
      rotation = rotate(mat4(1.0f), angle * scalar_factor, rotation_axis);
      return rotation;
    }
...
private:
    ...
    void ProjectOntoSurface(vec3& p) const {
      ...
      float crossover = pow(radius_, 2.0f) / 2.0f;
      if ((pow(p.x, 2.0f) + pow(p.y, 2.0f)) < crossover) {
        // inside the sphere
        p.z = sqrt(pow(radius_, 2) - (pow(p.x, 2.0f) + pow(p.y, 2.0f)));
      } else {
        // outside the sphere -> hyperbolic sheet
        p.z = (pow(radius_, 2.0f) / 2.0f) / (sqrt(pow(p.x, 2.0f) + pow(p.y, 2.0f)));
      }
    }
